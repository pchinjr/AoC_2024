  ...
  function isSafeReport(levels: number[]): boolean {
    ...
    for (let i = 1; i < levels.length; i++) {
      ...
      if (Math.abs(diff) < 1 || Math.abs(diff) > 3) {
        console.log(`Failed difference rule: ${levels[i - 1]} -> ${levels[i]} (diff: ${diff})`);
    ...
    if (!allIncreasing && !allDecreasing) {
      console.log("Failed consistency rule: Differences are neither all increasing nor all decreasing");
      return false;
    }
